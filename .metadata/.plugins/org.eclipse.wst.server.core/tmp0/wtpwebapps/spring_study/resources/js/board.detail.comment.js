        ...
        if(result.length > 0){
            ...
            for(let i=0; i<result.length; i++){
                ...
                if(id === result[i].writer){
                    add += `<button type="button" data-cno="${result[i].cno}" class="btn btn-outline-warning btn-sm cmtModBtn">댓글수정</button>`;
                    add += `<button type="button" data-cno="${result[i].cno}" class="btn btn-outline-danger btn-sm cmtDelBtn">댓글삭제</button>`;
                }
