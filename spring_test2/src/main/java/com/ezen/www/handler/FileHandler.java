...
@Slf4j
@Component // 사용자 클래스를 빈(bean)으로 등록.
public class FileHandler {
	private final String UP_DIR = "D:\\_myProjectRok\\_java\\_fileUpload";
	...
	public List<FileVO> uploadFiles(MultipartFile[] files){
		...
		//D:\\_myProjectRok\\_java\\_fileUpload\\2024\\05\\03
		...
		for(MultipartFile file : files) {
			...
			//D:\\_myProjectRok\\_java\\_fileUpload\\2024\\05\\03//uuid_name.jpg
			//실제 파일이 저장되려면 첫 경로부터 다 설정되어야 저장됨.
			File storeFile = new File(folders, fullFileName);
			
			try {
				file.transferTo(storeFile);
				//섬네일 저장 => 이미지만 썸네일 생성 가능
				//이미지인지 확인
